<template>
    <div class="box">
        <div class="header">
            <span>
                Peak Time Of accidents Per Factor
            </span>
        ...
        <div class="body">
            ...
            <p v-else class="no-data">Please select an interval and factor to view the data.</p>
        </div>
...
import { fetchPeakAccidentTimeStats } from '../../../../services/FactorsService';
...
export default {
    ...
    methods: {
        async fetchChartData() {
            ...
            this.loading = true;
            try {
                const res = await fetchPeakAccidentTimeStats(this.selectedInterval.value, this.selectedFactor.value);
                this.chartData = res;
                console.log("chart Data Peak time: ",this.chartData)
            } catch (err) {
                console.error(err);
            } finally {
                this.loading = false;
